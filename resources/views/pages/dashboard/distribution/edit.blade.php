                ...
                <form action="{{ route('distribution.update', $distribution->id) }}" method="POST" enctype="multipart/form-data">
                  ...
                  <div class="row">
                    ...
                    <div class="form-group">
                      ...
                      @foreach ($batchs as $batch)
                          <option value="{{ $batch->id }}" @if(old('tahfidz_house_id', $distribution->batch->id) === $batch->id) selected @endif>{{ $batch->batch }}</option>
                      @endforeach
                    </select>
                       @error('batch_id')
                  ...
                  <select class="form-control @error('tahfidz_house_id') is-invalid @enderror" id="tahfidz_house_id" name="tahfidz_house_id">
                      ...
                      @foreach ($tahfidzHouses as $tahfidzHouse)
                          <option value="{{ $tahfidzHouse->id }}" @if(old('tahfidz_house_id', $distribution->tahfidz_house_id) === $tahfidzHouse->id) selected @endif>{{ $tahfidzHouse->name }}</option>
                      @endforeach
                    </select>
                  ...
                  <div class="form-group">
                      ...
                      <input type="number" value="{{ old('total_rice') ?? $distribution->total_rice }}" class="form-control  @error('total_rice') is-invalid @enderror" id="total_rice" name="total_rice">
                    ...
                    <div class="col-4">
                      <div class="form-group">
                        ...
                        <input type="file" value="{{ old('image_url') }}" accept=".png.jpg.jpeg" class="form-control @error('image_url') is-invalid @enderror" id="image" name="image_url">
                        @error('image_url')
                        <small class="text-muted"><p class="text-danger">{{ $message }}</p></small>
                        @enderror
                      </div>
                      <img src="{{ url($distribution->image_url) }}" alt="Image Preview" class="rounded" style="width: 100%; height:180px;" id="image-preview">
                    </div>
